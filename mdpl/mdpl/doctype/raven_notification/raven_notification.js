...
// =============custon code for raven notification======================
frappe.ui.form.on("Raven Notification", {
	refresh: function(frm) {
	// 	if (frm.doc.document_type) {
	// 		var doctype = frm.doc.document_type;
	// 		var docname = frm.doc.name;
	// 		var link = frappe.urllib.get_full_url(`/desk#Form/${doctype}/${docname}`);
	// 		console.log(link); // prints the link to the document
	// 	}
	...
	validate: function(frm) {
		...
		//  below code is for pdf generation
		// pdf_doctype = frm.doc.document_type.toString().replace(' ', '%20');
		var pdf_link = window.location.host
		frm.doc.pdf_link = pdf_link;
		cur_frm.refresh_field("pdf_link")
	...
	// 							if (selected_fields[key]) {
	// 								if (key.startsWith('doctype_name_')) {
	// 									// Fields from the main doctype
	// 									custom_json.doctype_fields[key.replace('doctype_name_', '')] = selected_fields[key];
	// 								} else {
	// 									// Assuming child table fields are prefixed with the child table fieldname
	// 									var [child_table, fieldname] = key.split('_', 2);
	// 									if (!custom_json.child_table_fields[child_table]) {
	// 										custom_json.child_table_fields[child_table] = {};
	// 									}
	// 									custom_json.child_table_fields[child_table][fieldname] = selected_fields[key];
	// 								}
	// 							}
	// 						}
