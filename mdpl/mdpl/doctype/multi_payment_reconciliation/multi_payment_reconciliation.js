...
// frappe.ui.form.on("Multi Payment Reconciliation", {
// 	refresh(frm) {
...
erpnext.accounts.MultiPaymentReconciliationController = class MultiPaymentReconciliationController extends (
	...
	refresh() {
		...
		this.frm.add_custom_button(__("Auto Reconciled"), () => {
			this.frm.trigger("get_unreconciled_entries")
				.then(() => {
					// Wait for get_unreconciled_entries to complete
					if (this.frm.doc.invoices.length && this.frm.doc.payments.length) {
						this.frm.trigger("allocate").then(() =>{
		...
		this.frm.change_custom_button_type(__("Auto Reconciled"), null, "primary");
		// }
	...
	party_type() {
		...
		frappe.db.get_value("Company",frappe.defaults.get_default("company"),['default_receivable_account','default_payable_account'] , (r) => {
			if (this.frm.doc.party_type == "Customer"){
				this.frm.doc.supplier_name = ''
				this.frm.doc.receivable_payable_account = r.default_receivable_account
				this.frm.refresh_fields('receivable_payable_account')
			} 
			if (this.frm.doc.party_type == "Supplier"){
				this.frm.doc.customer_name = ''
				this.frm.doc.receivable_payable_account = r.default_payable_account
				this.frm.refresh_fields('receivable_payable_account')
			} 
		})
...
extend_cscript(cur_frm.cscript, new erpnext.accounts.MultiPaymentReconciliationController({ frm: cur_frm }));
